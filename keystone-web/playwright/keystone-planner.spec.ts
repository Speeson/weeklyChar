...
test('session entry starts with safe defaults, enforces participants and renders server Top 3', async ({ page }) => {
  ...
  await expect(dialog.getByRole('button', { name: 'Calcular Top 3' })).toBeDisabled()
  ...
  await dialog.getByRole('button', { name: 'Calcular Top 3' }).click()
...
test('empty, unconfigured, 422 and access-lost responses are explicit and recoverable', async ({ page }) => {
  ...
  const dialog = page.getByRole('dialog', { name: 'Planificar sesión' })
  await dialog.getByRole('checkbox', { name: /^Healer/u }).check()
  const calculate = dialog.getByRole('button', { name: 'Calcular Top 3' })
  await calculate.click()
  await expect(dialog.getByText('No hay piedras disponibles')).toBeVisible()
  await dialog.getByRole('button', { name: 'Recalcular Top 3' }).click()
