...
const previewTracks = ["Explorer", "Adventurer", "Veteran", "Champion", "Hero", "Myth"];
const equipment: EquipmentItem[] = slots.map((slotName, index) => ({
  ...
  ...(index < previewTracks.length ? { upgrade: { track: previewTracks[index], currentLevel: 2, maxLevel: 6 } } : {}),
}));

const hunterNeckGem = { itemId: 240892, itemLink: "item:240892", name: "Flawless Masterful Peridot", iconFileID: 7549142 };
const hunterEquipment: EquipmentItem[] = equipment
  .filter(item => item.slotId !== 16)
  .map(item => item.slotId === 2 ? {
    ...item,
    itemId: 268265,
    itemName: "Aqirbane Reliquary",
    itemLevel: 321,
    iconFileID: null,
    iconPath: "https://wow.zamimg.com/images/wow/icons/large/inv_121_jewelry_neck03_green.jpg",
    gems: [{ ...hunterNeckGem }, { ...hunterNeckGem }],
    bonusIds: [6652, 13668, 13334, 13987, 12846],
    enchant: null,
    setId: null,
  } : { ...item });
...
export function charactersPreview(): Character[] {
  ...
  return base.map(([name, wowClass], index) => ({
    id: `characters-${index}`, name, realm: index >= 7 ? "Sanguino" : "Zul'jin", region: "eu", wowAccount: index === 8 ? "WOW Account 2" : "WoW Account", wowClass: index === 1 ? "Hunter" : wowClass, avatarUrl: null, ilvl: 315.69, rioScore: 3000, currentKeystone: null, keystoneDisplay: "—",
    ...(index === 0 ? {
      equipment: { averageItemLevel: 315.69, setPieces: [{ setId: 35, count: 2 }, { setId: 36, count: 3 }], tierPieces: [{ tier: 35, count: 2 }, { tier: 36, count: 3 }], items: equipment },
      ...
      mythicPlusSeason: { rating: 3000, dungeons: MIDNIGHT_SEASON_2_DUNGEONS.map((dungeon, runIndex) => ({ challengeMapId: dungeon.id, level: [12, 10, 13, 11, 9, 12, 10, 11][runIndex], upgradeLevel: [2, 1, 3, 2, 1, 2, 1, 2][runIndex], timed: true, rating: [322, 305, 292, 320, 276, 307, 295, 336][runIndex] })) },
      vault: {
        raid: { slots: [{ progress: 2, threshold: 2, unlocked: true, rewardItemLevel: 316, rewardUpgradeTrack: "Hero", encounters: [{ encounterID: 7001, name: "Reina Ansurek", bestDifficulty: 16 }] }, { progress: 2, threshold: 4 }, { progress: 2, threshold: 6 }] },
        dungeons: { completedRuns: { heroic: 0, mythic: 0, mythicPlus: 8 }, topRuns: MIDNIGHT_SEASON_2_DUNGEONS.map((dungeon, runIndex) => ({ level: [12, 12, 11, 11, 11, 11, 10, 10][runIndex], mapChallengeModeID: dungeon.id, name: dungeon.nameEs })), slots: [{ level: 12, progress: 8, threshold: 1, unlocked: true, rewardItemLevel: 318, rewardUpgradeTrack: "Myth" }, { level: 11, progress: 8, threshold: 4, unlocked: true, rewardItemLevel: 315, rewardUpgradeTrack: "Hero" }, { level: 10, progress: 8, threshold: 8, unlocked: true, rewardItemLevel: 311, rewardUpgradeTrack: "Hero" }] },
        world: { tierProgress: [{ activityTierID: 301, difficulty: 8, numPoints: 4 }, { activityTierID: 302, difficulty: 7, numPoints: 2 }], slots: [{ progress: 6, threshold: 2, unlocked: true, rewardItemLevel: 315, rewardUpgradeTrack: "Hero" }, { progress: 6, threshold: 4, unlocked: true, rewardItemLevel: 311, rewardUpgradeTrack: "Hero" }, { progress: 6, threshold: 8, unlocked: false }] },
      },
