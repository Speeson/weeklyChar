...
function ProgressPanels({ character, language }: { character: Character; language: "es" | "en" }) {
  ...
  return <><section className="characters-panel vault-panel"><h2>{copy.greatVault}</h2>{vaultNames.map(([name, label]) => { const data = category(name); const slots = (Array.isArray(data.slots) ? data.slots : []).map(record).sort((left, right) => number(left.threshold) - number(right.threshold)); const max = name === "Raids" ? 6 : 8; const progress = Math.min(max, Math.max(0, ...slots.map(slot => number(slot.progress)))); return <div key={name}><span>{label}</span><i>{[0, 1, 2].map(index => { const slot = slots[index]; const details = slot ? detailLines(name, data, slot) : []; const reward = slot?.unlocked ? formatVaultReward(slot.rewardItemLevel, slot.rewardUpgradeTrack, language) : null; return <span className="vault-slot" key={index} tabIndex={slot ? 0 : -1}><b>{slot?.unlocked ? number(slot.level) || "✓" : "—"}</b>{slot ? <span className="vault-slot__tooltip" role="tooltip"><strong>{language === "es" ? "Desbloquear recompensa" : "Unlock reward"}</strong><small><span>{number(slot.progress)} / {number(slot.threshold)}</span>{reward ? <b className="vault-slot__reward"><UpgradeTrackIcon track={slot.rewardUpgradeTrack} />{reward}</b> : null}</small>{details.length ? details.map((detail, detailIndex) => <em className={detail.completed ? "is-completed" : undefined} key={`${detail.text}-${detailIndex}`}><span>{detail.text}</span></em>) : <em>{language === "es" ? "Sin actividad registrada" : "No recorded activity"}</em>}</span> : null}</span>; })}</i><strong>{progress} / {max}</strong></div>; })}</section>
