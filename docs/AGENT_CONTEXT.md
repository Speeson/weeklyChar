...
- Worker workflow: `.github/workflows/deploy-worker.yml` validates `npm run typecheck` and `npm test`; independent production operations remain guarded, while a backend-dependent Client release forces impacted D1 migrations, then Worker deploy, then a non-destructive production smoke of `/api/health` and the authenticated Selector route. Production smoke uses the environment secret `WORKER_SMOKE_BYPASS_TOKEN` as `X-KeystoneSync-Smoke-Token`; the matching Cloudflare exception must be limited to those smoke paths. A confirmed `recover_worker_readiness` orchestrator dispatch repeats only smoke after a previously successful migration/deploy and still gates Client publication.
