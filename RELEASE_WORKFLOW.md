...
- No editar manualmente `keystone-client/addon/KeystoneSync/`; esa copia se genera/sincroniza desde `Speeson/KeystoneSync`.
- Para refrescar el bundle del cliente desde un checkout local del addon, ejecutar `python scripts/sync_addon.py --source <path-to-Speeson-KeystoneSync>` y despues `python scripts/check_addon_sync.py --source <path-to-Speeson-KeystoneSync>`.
